...
export function CreatePlanModal({ 
  ...
  const handleSave = async (data: CreatePlanFormData) => {
    // Check for duplicate plan names
    if (existingPlans.some(p => p.name.toLowerCase() === data.name.toLowerCase())) {
      form.setError('name', { message: 'Plan name already exists' });
      return;
    }
      ...
      <DialogContent className="max-w-4xl max-h-[95vh] overflow-hidden">
        <DialogHeader>
          <DialogTitle className="flex items-center space-x-3">
            <div className="w-10 h-10 bg-gradient-to-br from-green-500/20 to-blue-600/20 rounded-lg flex items-center justify-center border border-border/50">
              <Plus className="h-5 w-5 text-green-500" />
            </div>
            <div>
              <div className="text-xl font-bold">Create Subscription Plan</div>
          ...
          <DialogDescription>
            Create a new subscription plan with pricing, features, and usage limits
          </DialogDescription>
        </DialogHeader>
