...
{
    public Task<int> GetCountByConsumerIdAsync(Guid consumerId, CancellationToken ct = default)
    {
        return context.PaymentMethods
            .CountAsync(pm => pm.ConsumerId == consumerId, ct);
    }

    public Task<PaymentMethod?> GetByStripePaymentMethodId(string stripePaymentMethodId, CancellationToken ct = default)
    {
        return context.PaymentMethods
            .AsNoTracking()
